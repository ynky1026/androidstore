...
public class CameraImgActivity extends ImgAttachDetailCheck {
	...
	protected void init() {
		...
		bitmapPath = getIntent().getExtras().getString(
				ConstantValue.CAMERA_BIT_TMP_PATH);
		bitmapFile = new File(bitmapPath);
		if (bitmapFile != null && bitmapFile.exists()) {
			bitmap = BitmapFactory.decodeFile(bitmapPath);
			siv.setImageBitmap(bitmap);
		}
		fileName = bitmapPath.substring(bitmapPath.lastIndexOf("/") + 1,
				bitmapPath.length());
	}
	...
	protected void save() {
		...
		it.putExtra(ConstantValue.FILE_PATH, bitmapPath);
